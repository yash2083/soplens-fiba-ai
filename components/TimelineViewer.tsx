...
import { type AnalysisResult, formatTimestamp } from "@/services/fibaApi";
...
export function TimelineViewer({ result }: Props) {
  ...
  const events: TimelineEvent[] = ([
    { label: "Start", timestamp: startMs, type: "start" as const, description: "Video analysis begins" },
    ...result.key_frames.map((_, i): TimelineEvent => ({
      label: `Frame ${i + 1}`,
      timestamp: startMs + (durationMs / (result.key_frames.length + 1)) * (i + 1),
      type: "keyframe",
      description: `Key moment ${i + 1} detected`,
    })),
    {
      label: result.action_label,
      timestamp: startMs + durationMs * 0.5,
      type: "action" as const,
      description: result.evidence || `${result.action_label} action detected`,
    },
    { label: "End", timestamp: endMs, type: "end" as const, description: `Analysis complete in ${result.processing_time_s?.toFixed(1)}s` },
  ] as TimelineEvent[]).sort((a, b) => a.timestamp - b.timestamp);
    ...
    <View style={s.container}>
      ...
      <View style={s.track}>
        ...
        {events.map((event, index) => {
          const progress = durationMs > 0 ? (event.timestamp - startMs) / durationMs : 0;
          const leftPercent = Math.max(0, Math.min(progress * 100, 100));
          return (
            <TouchableOpacity
              key={index}
              style={[s.dot, { left: `${leftPercent}%` as unknown as number }]}
              onPress={() => setActiveEvent(activeEvent === index ? null : index)}
            >
              <View
                style={[
                  s.dotInner,
                  { backgroundColor: getEventColor(event.type), width: event.type === "action" ? 14 : 10, height: event.type === "action" ? 14 : 10, borderRadius: event.type === "action" ? 7 : 5 },
        ...
        {events.map((event, index) => (
          ...
          >
            <View style={[s.eventIcon, { backgroundColor: getEventColor(event.type) + "20" }]}>
              <Ionicons
                name={getEventIcon(event.type) as "play-circle-outline"}
                size={15}
            ...
            <View style={s.eventContent}>
              <View style={s.eventHeader}>
                ...
                <Text style={s.eventTime}>{formatTimestamp(event.timestamp)}</Text>
              </View>
              {event.description && (
